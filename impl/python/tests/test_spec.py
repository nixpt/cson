...
def raises(label, text, needle):
    ...
    checks += 1
    try:
        cson.loads(text)          # parse + project, so projection errors count too
...
ok("no metadata -> bare value", cson.project(cson.parse("a: 1").root), {"a": 1.0})
ok("array element metadata wraps", cson.project(cson.parse("a: [1 ~0.5, 2]").root),
   {"a": [{"$value": 1.0, "$confidence": 0.5}, 2.0]})
ok("absent vs explicit 1.0 visible in projection",
   cson.project(cson.parse("a: 1\nb: 1 ~1.0").root),
   {"a": 1.0, "b": {"$value": 1.0, "$confidence": 1.0}})
ok("value-only helper drops metadata", cson.project_values(doc.root), {"t": 21.5})
